...
var socialScroll = (0, _jquery2.default)('.social-mobile'),
    articleScroll = (0, _jquery2.default)('.other-article');

// --- ce bloc est a supprimer apres validation inté ----
(0, _jquery2.default)(articleScroll).css('opacity', 0);
(0, _jquery2.default)('.btn-article').on("click", function () {
  (0, _jquery2.default)(socialScroll).css('opacity', 0);
  (0, _jquery2.default)(articleScroll).css('opacity', 1);
});
(0, _jquery2.default)('.btn-social').on("click", function () {
  (0, _jquery2.default)(socialScroll).css('opacity', 1);
  (0, _jquery2.default)(articleScroll).css('opacity', 0);
});
// ---- fin du bloc à supprimer -----


if ((0, _jquery2.default)(socialScroll).length != 0) {
  showScrollUp(socialScroll);

  // ------ la ligne suivante est à supprimer apres validation inté -----
  showScrollUp(articleScroll);
  // ----- fin de la ligne à supprimer ----- 
} else if ((0, _jquery2.default)(articleScroll).length != 0) {
  showScrollUp(articleScroll);
}

function showScrollUp(container) {
  var container = container;
  var previousScroll = 0;
  (0, _jquery2.default)(window).scroll(function (event) {
    var scroll = (0, _jquery2.default)(this).scrollTop();
    if (scroll > previousScroll) {
      (0, _jquery2.default)(container).css('bottom', '-100%');
    } else {
      (0, _jquery2.default)(container).css('bottom', '0');
    }
    previousScroll = scroll;
  });
};

(0, _jquery2.default)(".next-article .article-content").load("youhou.html #header-article");

(0, _jquery2.default)('.next-article #header-article').ready(function () {
  var nextArticle = document.getElementById('header-article');
  var hammertime = new Hammer(nextArticle);
  hammertime.on('pan tap', function () {
    (0, _jquery2.default)(nextArticle).find('.title-h1').css('margin-bottom', '20px');
    (0, _jquery2.default)(nextArticle).find('.social-inline').css('transform', 'translateY(0px)');
    (0, _jquery2.default)('.article-content-body').load('youhou.html #body-article');
  });
});
